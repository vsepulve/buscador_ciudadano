...
if($textoQuery){
	...
	if($results[$consulta_actual]["TOTAL"]==200){
...
echo "<form name=form1 action=index.php?>\n";
...
if($countSuggestions[0]>0 ){
	...
	for($i=0; $i<$suggestions[0]["TOTAL"]; $i++){
		...
		echo "<a href=index.php?query=".str_replace(" ", "+", $suggestions[0][$i]["NEW_QUERY"])."&and_or=$and_or&accents=$accents&feedback=$feedback&docs_page=$docs_page&id_grupo=$id_grupo>";
...
if($textoQuery==null){
	...
	for($i=0; $i<$numeroPaginas; $i++){
		if($i==$page)
			...
			echo "<a href=index.php?query=".str_replace(" ", "+", $textoQuery)."&and_or=$and_or&accents=$accents&page=$i&feedback=$feedback&docs_page=$docs_page&id_grupo=$id_grupo>".($i+1)."</a> &nbsp;";
	}
...
if($countResults[$consulta_actual]>0){
	...
	for($i=0; $i<$countResults[$consulta_actual]; $i++){
		...
		echo "<a href=index.php?query=".$queryLista."&and_or=$and_or&accents=$accents&page=$page&feedback=1&docs_page=$docs_page&doc_id=".$results[$consulta_actual][$i]["DOC_ID"]."&id_grupo=$id_grupo> [P&aacute;ginas similares]</a>";
		echo "</td>\n";
